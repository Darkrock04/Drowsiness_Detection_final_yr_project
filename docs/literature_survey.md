...
This document outlines the academic context of the Driver Monitoring System, detailing the flaws in existing research implementations and explaining how our custom-trained model resolves them.
...
2.  **Heavy Convolutional Neural Networks (CNNs):** Recent papers have shifted to using deep CNNs (like ResNet50, VGG16, or heavy PyTorch architectures) to classify driver states. While highly accurate, these models require massive computational overhead, usually demanding discrete NVIDIA GPUs (CUDA) to process video frames in real-time.
3.  **Fragmented Multi-Model Systems:** To detect both physiological states (drowsiness) and physical objects (phones/cigarettes), researchers often run a facial-landmark detector concurrently with an object detector. This doubles the computational cost and makes the system impractical for embedded hardware.
...
Training a neural network entirely from scratch to recognize complex micro-expressions requires millions of images and months of GPU computing. Instead, we utilized **Transfer Learning**. We took a foundational pre-trained object detection architecture—which already possessed a deep understanding of generic shapes, edges, and lighting—and fine-tuned it on a highly specific 52,000-image dataset. This allowed our custom model (`final_yr_drowsiness_model.tflite`) to rapidly learn how to natively classify 5 simultaneous driver infractions (Drowsiness, Yawning, Distraction, Phone Usage, Smoking) as distinct classes, eliminating the need for fragile mathematical tracking.

### Training Environment
Our model was trained on **Kaggle** using dual T4 GPUs over 50 epochs (~9 hours of training). Kaggle was chosen because it provides free, high-performance GPU compute specifically suited for deep learning workloads, without requiring local hardware investment.
...
Our custom-trained model was exported to a **TensorFlow Lite (`.tflite`)** format. During this export, we applied **Float16 (`FP16`) Quantization**. 
